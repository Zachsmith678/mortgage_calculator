...
jQuery(document).ready(function($){
    ...
    $range.ionRangeSlider({
		...
		onChange: function (data) {
			...
			if(sliderId == 'price-slider'){
				$downInstance = $("#dPayment-slider").data("ionRangeSlider");
				$('#dPayment').val(addCommas((inputVal('#price')*inputVal('#downPayPercent')/100).toFixed(0)));
				
			}
			if(sliderId == 'dPayment-slider'){
				$('#dPayment').val(addCommas((inputVal('#price')*inputVal('#downPayPercent')/100).toFixed(0)));
    ...
    $('.input').bind('input',function(){
		...
		if( id == 'downPayPercent'){
			...
			if(interest > 100){
				...
				$intInstance = $("#interest-slider").data("ionRangeSlider");
				$intInstance.update({
					from: interest
				}); 
			}
			...
			var labels = [
				'Principal & interest <samp class="label-value">$'+addCommas(pmt.toFixed(2))+'</samp>',
				'Property taxes<samp class="label-value">$'+addCommas(tax.toFixed(2))+'</samp>',
				'Home insurance<samp class="label-value">$'+addCommas(insur.toFixed(2))+'</samp>',
				'HOA<samp class="label-value">$'+addCommas(hoa.toFixed(2))+'</samp>',
				'Mortgage Insurance<samp class="label-value">$'+addCommas(mortgageInsu.toFixed(2))+'</samp>',
				'Other Payment<samp class="label-value">$'+addCommas(otherPay.toFixed(2))+'</samp>',
			];
